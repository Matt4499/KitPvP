...
public class Kit {
    ...
    public Ability getAbilityFromActivator(ItemStack activator) {
        for (Ability ability : abilities) {
            if (activator.getType() == ability.getActivator().getType()) {
                ...
                assert meta != null;
                if (meta.getDisplayName().equals(Objects.requireNonNull(ability.getActivator().getItemMeta()).getDisplayName())) {
                    return ability;
                }
            }
        }
    ...
    public void toResource(Resource resource) {
        ...
        abilities.forEach(ability -> ability.toResource(resource, "Abilities."));
        resource.save();
    }
