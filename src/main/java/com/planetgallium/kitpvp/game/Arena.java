...
//		}
    ...
    public void giveItems(Player p) {
        ...
        for (String identifier : items.getKeys(false)) {
            ...
            if (config.getBoolean(itemPath + ".Enabled")) {
                ItemStack item = new ItemStack(Objects.requireNonNull(XMaterial.matchXMaterial(Objects.requireNonNull(config.getString(itemPath + ".Material"))).get().parseMaterial()));
    ...
    public void toSpawn(Player p, String arenaName) {
        if (config.contains("Arenas." + arenaName)) {
            ...
            p.sendMessage(Objects.requireNonNull(resources.getMessages().getString("Messages.Error.Arena")).replace("%arena%", arenaName));
        }
    }
    public void updateScoreboards(Player p, boolean hide) {
        Scoreboard board = Objects.requireNonNull(Bukkit.getScoreboardManager()).getNewScoreboard();
        Infoboard scoreboard = new Infoboard(board, Objects.requireNonNull(resources.getScoreboard().getString("Scoreboard.General.Title")));
        if (!hide) {
    ...
    public boolean isCombatActionPermittedInRegion(Player p) {
        if (plugin.hasWorldGuard()) {
            ...
            p.sendMessage(Objects.requireNonNull(resources.getMessages().getString("Messages.Error.PVP")));
            return false;
        }
        return true;
    }
